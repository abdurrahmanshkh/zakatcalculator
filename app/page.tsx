...
const SectionHeader = ({ icon: Icon, title, isOpen, toggle, total, currency }) => (
  <button 
    onClick={toggle}
...
const InputGroup = ({ label, sublabel, value, onChange, placeholder = "0.00", tooltip, currencySymbol }) => (
...
const Toggle = ({ label, active, onToggle, tooltip }) => (
...
export default function App() {
  ...
  const [goldPrice, setGoldPrice] = useState(7200.00); 
  const [silverPrice, setSilverPrice] = useState(92.00); 
  ...
  useEffect(() => {
    const symbols = { INR: '₹', USD: '$', GBP: '£', EUR: '€', AED: 'د.إ', SAR: '﷼' };
  ...
  const fetchRates = async () => {
    try {
      ...
      // Auto-adjust default metal prices based on currency selection if reasonable
      // Note: This is a rough estimation based on INR standard. 
      // Ideally user enters local market price.
      const rate = data.rates[currency] || 1;
      // If we switched currency, we might want to scale the prices? 
      // For now, let's just alert the user to check prices.
    } catch (error) {
  ...
  const updateLiability = (key, value) => setLiabilities(prev => ({ ...prev, [key]: value }));
  ...
  const calculations = useMemo(() => {
    ...
    // 2. Gold & Silver
    let goldValue = 0;
    let silverValue = assets.silverGrams * silverPrice;
    ...
    if (preciousMetalsTotal > 0) breakdown.push({ label: 'Zakatable Gold & Silver', amount: preciousMetalsTotal });
    ...
    const hasMixedAssets = (cashTotal + businessTotal + investmentsTotal + silverValue) > 0;
    const applicableNisab = hasMixedAssets ? silverNisab : (assets.goldGrams > 0 ? goldNisab : silverNisab);
      ...
      <header className="bg-emerald-900 text-white shadow-lg no-print">
        ...
        <div className="bg-emerald-950 px-4 py-2 text-xs md:text-sm text-emerald-200 border-b border-emerald-800">
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
            <span>In Collaboration with <strong><a href="https://www.ilannoor.org/" target="_blank" rel="noopener noreferrer" className="hover:text-white underline decoration-emerald-600 underline-offset-2">Il An Noor Foundation</a></strong></span>
            <span className="flex items-center gap-4">
               <a href="mailto:ilannoorirc@gmail.com" className="hover:text-white flex items-center gap-1"><Mail size={12}/> ilannoorirc@gmail.com</a>
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex flex-col gap-3 w-full md:w-auto">
               ...
               <div className="bg-emerald-800/50 p-3 rounded-lg border border-emerald-600/30 text-sm">
                  <p className="text-emerald-200 text-xs font-semibold uppercase tracking-wider mb-2">Market Rates (Per Gram)</p>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="no-print">
              <SectionHeader 
                ...
                total={(assets.silverGrams * silverPrice) + (fiqh === 'hanafi' || !assets.goldJewelryUsage ? assets.goldGrams * goldPrice : 0)}
                currency={currencySymbol}
              />
              {sections.gold && (
                <div className="p-6 bg-white animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                    ...
                    />
                    {assets.goldJewelryUsage && fiqh !== 'hanafi' && <p className="text-xs text-emerald-600">Exempt from Zakat based on {fiqh} fiqh.</p>}
                    {assets.goldJewelryUsage && fiqh === 'hanafi' && <p className="text-xs text-amber-600">Zakatable in Hanafi fiqh.</p>}
                  </div>
                </div>
              )}
            </Card>
            ...
            <Card className="no-print">
              <SectionHeader 
                ...
                total={(assets.stocksStrategy === 'active' ? assets.stocksValue : assets.stocksValue * 0.25) + assets.cryptoValue + (assets.pensionAccess ? assets.pensionAmount * 0.25 : 0)}
                currency={currencySymbol}
              />
              {sections.investments && (
                <div className="p-6 bg-white animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="flex gap-4 mb-4">
                     <button onClick={() => updateAsset('stocksStrategy', 'passive')} className={`flex-1 py-2 text-xs rounded-md border ${assets.stocksStrategy === 'passive' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-slate-200'}`}>Passive (25%)</button>
                     <button onClick={() => updateAsset('stocksStrategy', 'active')} className={`flex-1 py-2 text-xs rounded-md border ${assets.stocksStrategy === 'active' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-slate-200'}`}>Trader (100%)</button>
                  </div>
                  <InputGroup label="Crypto Value" value={assets.cryptoValue} onChange={(v) => updateAsset('cryptoValue', v)} currencySymbol={currencySymbol} />
                </div>
              )}
            </Card>
            ...
            <Card className="no-print">
              ...
              {sections.liabilities && (
                <div className="p-6 bg-white animate-in slide-in-from-top-2 duration-200">
                   {fiqh === 'shafii' && <p className="text-xs text-red-500 mb-2">Debts are not deductible in Shafi'i fiqh.</p>}
                   <InputGroup 
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-emerald-100 overflow-hidden sticky top-6">
              <div className="bg-slate-900 text-white p-6 relative overflow-hidden">
                ...
                <p className="text-slate-400 text-sm relative z-10">Based on {fiqh.charAt(0).toUpperCase() + fiqh.slice(1)} Fiqh</p>
              </div>
              ...
              <div className="p-6">
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-semibold text-slate-800">{currencySymbol} {calculations.zakatableAssets.toLocaleString()}</span>
                  </div>
                  ...
                  {calculations.deductibleLiabilities > 0 && (
                     <div className="flex justify-between items-center text-sm text-red-600">
                       ...
                       <span>- {currencySymbol} {calculations.deductibleLiabilities.toLocaleString()}</span>
                     </div>
                  )}
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-slate-900 text-lg">{currencySymbol} {calculations.netWorth.toLocaleString()}</span>
                  </div>

                  {/* Nisab Status */}
                  <div className={`p-4 rounded-lg text-center border ${calculations.isEligible ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                    <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Threshold (Nisab)</span>
                    <span className="text-slate-700 font-medium block mb-2">{currencySymbol} {calculations.applicableNisab.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                  ...
                  {calculations.isEligible && (
                    <div className="mt-4 pt-4 border-t border-dashed border-emerald-200">
                      ...
                      <p className="text-center text-4xl font-bold text-emerald-700">{currencySymbol} {calculations.zakatPayable.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                    </div>
                  )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 no-print">
               ...
               <div className="space-y-3">
                 <a href="https://wa.me/918104998499" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-2.5 bg-emerald-50 text-emerald-700 rounded-lg font-medium hover:bg-emerald-100 transition-colors border border-emerald-200">
                   <MessageCircle size={18} /> WhatsApp
                 </a>
                 <a href="tel:+918104998499" className="flex items-center justify-center gap-2 w-full py-2.5 bg-white text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors border border-slate-200">
      ...
      <footer className="bg-slate-900 text-slate-400 py-12 mt-12 no-print">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-sm">
          ...
          <div className="lg:col-span-2 md:text-right">
             ...
             <a href="https://abdurrahmanshkh.vercel.app/" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300 font-medium inline-flex items-center gap-1 mb-4">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
           <div>
             ...
             <div className="grid grid-cols-2 gap-y-1 text-sm">
               <span className="text-slate-600">Fiqh:</span> <span className="font-medium capitalize">{fiqh}</span>
               <span className="text-slate-600">Currency:</span> <span className="font-medium">{currency}</span>
               <span className="text-slate-600">Gold Price:</span> <span>{currencySymbol}{goldPrice}/g</span>
               <span className="text-slate-600">Silver Price:</span> <span>{currencySymbol}{silverPrice}/g</span>
             </div>
           </div>
           <div>
             <h3 className="font-bold border-b border-slate-300 pb-1 mb-2">Result</h3>
             <div className="grid grid-cols-2 gap-y-1 text-sm">
               <span className="text-slate-600">Net Wealth:</span> <span className="font-bold">{currencySymbol}{calculations.netWorth.toLocaleString()}</span>
               <span className="text-slate-600">Nisab Threshold:</span> <span>{currencySymbol}{calculations.applicableNisab.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
          ...
          <tbody>
            {calculations.breakdown.map((item, idx) => (
              <tr key={idx} className="border-b border-slate-100">
                ...
                <td className="p-3 text-right font-medium">{currencySymbol} {item.amount.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
              </tr>
            ))}
            {calculations.deductibleLiabilities > 0 && (
              <tr className="border-b border-slate-100 text-red-600">
                <td className="p-3">Less: Deductible Liabilities</td>
                <td className="p-3 text-right font-medium">- {currencySymbol} {calculations.deductibleLiabilities.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
              </tr>
            )}
          </tbody>
          <tfoot>
...
const HelpCircleIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-600"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>
)